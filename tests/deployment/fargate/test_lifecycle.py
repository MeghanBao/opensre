...
from platform.deployment.fargate.aws_s3_files import (
    S3FilesAccessPoint,
    S3FilesAdapter,
)
...
from platform.deployment.fargate.lifecycle import FargateLifecycle
from platform.deployment.fargate.lifecycle_errors import (
    LifecycleCapacityError,
    LifecycleOperationError,
    LifecycleValidationError,
)
from platform.deployment.fargate.lifecycle_models import FargateLifecycleConfig
...
def _config() -> FargateLifecycleConfig:
    return FargateLifecycleConfig(
...
def test_zero_argument_factory_uses_real_environment_and_standard_aws_clients(
    ...
    import platform.deployment.fargate.lifecycle as lifecycle_module
    ...
    lifecycle = lifecycle_module.create_lifecycle_service_from_environment()

    assert isinstance(lifecycle, FargateLifecycle)
...
def test_factory_environment_has_no_demo_fallbacks(
    ...
    with pytest.raises(RuntimeError, match="OPENSRE_GATEWAY_IMAGE"):
        FargateLifecycleConfig.from_environment()
