...
| `platform/`                                   | Cross-cutting platform services: guardrails, masking, sandbox, analytics, auth, notifications, observability, harness ports (`platform/harness_ports.py`), shared multi-tenant contracts (`platform/deployment_contracts/`; the Fargate deployment itself lives in the separate private `opensre-infra-aws` repository), and EC2 AWS helpers (`platform/deployment_ec2/`). |
...
- `platform/deployment_contracts/` — shared multi-tenant deployment contracts
 (models and gateway-facing repository ports). The Fargate deployment itself —
 fleet Terraform (`modules/fargate_fleet`), Lambda runtimes
 (`lambda_control_plane/`, `lambda_public_forwarder/`), and shared / team stacks
 under `stacks/` — lives in the separate private
 `https://github.com/Tracer-Cloud/opensre-infra-aws.git` repository; clone it
 separately. Makefile: `make cdk-verify OPENSRE_INFRA_AWS_DIR=/path/to/checkout`
 (bundle path check).
