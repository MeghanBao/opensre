...
tools (never in tool-call traces). Shared client: `integrations/slack/bot_api.py`.
...
| `channels:history` | `slack_read_messages` in public channels |
...
| `im:read` / `mpim:read` | List DMs when resolving names |
| `users:read` | `slack_list_team_members` |
| `search:read` | `slack_search_messages` |
| `channels:join` | `slack_join_channel` (public channels) |
