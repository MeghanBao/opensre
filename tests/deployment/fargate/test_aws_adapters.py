...
from platform.deployment.fargate.aws_discovery import ExistingInfrastructureDiscovery
...
from platform.deployment.fargate.aws_iam import (
...
from platform.deployment.fargate.aws_types import AwsResourceConfigurationError
...
def test_discovers_only_explicit_existing_infrastructure() -> None:
    ...
    result = ExistingInfrastructureDiscovery(ec2, ecr, s3).discover(
        vpc_id="vpc-1",
...
def test_rejects_unsafe_existing_infrastructure(bad_setting: str) -> None:
    ...
    with pytest.raises(AwsResourceConfigurationError):
        ExistingInfrastructureDiscovery(ec2, ecr, s3).discover(
            vpc_id="vpc-1",
            subnet_ids=("subnet-a", "subnet-b"),
            ecr_repository_name="opensre",
            s3_bucket_name="opensre-files",
